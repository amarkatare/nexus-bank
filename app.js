...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Handle Supabase auth redirect (password reset, email confirm, etc.)
  // Supabase puts tokens in the URL hash e.g. #access_token=...&type=recovery
  const hashParams = new URLSearchParams(window.location.hash.replace('#', ''));
  const urlType = hashParams.get('type');

  if (urlType === 'recovery') {
    // Password reset link clicked — show reset page immediately
    // Supabase will fire PASSWORD_RECOVERY event shortly after
    showPage('reset');
  ...
  sb.auth.onAuthStateChange(async (event, session) => {
    if (event === 'PASSWORD_RECOVERY') {
      // User clicked the reset link in email — show reset page
      showPage('reset');
    } else if (event === 'SIGNED_IN' && session) {
      // Only go to dashboard if NOT coming from a password reset
      const isReset = document.getElementById('reset-page')?.classList.contains('active');
      if (isReset) return; // Stay on reset page
      currentUser = session.user;
      await loadProfile();
      showPage('dashboard');
      loadDashboardData();
...
async function handleResetPassword() {
  ...
  try {
    // Make sure we have an active session from the recovery link
    const { data: { session } } = await sb.auth.getSession();
    if (!session) {
      showAlert('reset-alert', 'error', '❌ Reset session expired. Please request a new reset link.');
    ...
    if (error) {
      ...
      showAlert('reset-alert', 'success', '✅ Password reset successfully! Redirecting to login...');
    ...
    showAlert('reset-alert', 'error', '❌ An unexpected error occurred. Please try again.');
  }
